...
for x in range(0,num):
  ...
  tickers2.append(stockA)

def import_stock_data(tickers, start = '2010-1-1', end = datetime.today().strftime('%Y-%m-%d')):
    data = pd.DataFrame()
    if len([tickers]) ==1:
        data[1] = wb.DataReader(tickers, data_source='yahoo', start = start)['Adj Close']
...
ret_sim_df = monte_carlo(tickers2, days_forecast= days_to_forecast, iterations=simulation_trials,  start_date=start, plotten=False)   
